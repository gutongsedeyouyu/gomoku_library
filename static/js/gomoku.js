 ...
 *
 * Public:
 *     Gomoku(character, upDownReflected, leftRightReflected, rotateTimes)
 *     showFreshNew()
 ...
 **************************************************************************************************/
function Gomoku(character, upDownReflected, leftRightReflected, rotateTimes) {
...
Gomoku.prototype.showFreshNew = function() {
    this.__reinitializeMoves();
    this.__updateUi();
}

Gomoku.prototype.loadLibrary = function(libraryId, title, blackPlayerName, whitePlayerName, manual) {
    this.showFreshNew();
...
Gomoku.prototype.saveLibrary = function() {
    ...
    var apiRequest = new ApiRequest('/api/library/save');
    apiRequest.send({
                        'id':this.libraryId,
                    ...
                    function(result) {
                        __gomoku__.libraryId = result['id'];
                        alert('保存成功');
                    },
...
Gomoku.prototype.playNewMoveAt = function(x, y) {
...
Gomoku.prototype.playPassMove = function() {
    this.playNewMoveAt(-1, -1);
}

Gomoku.prototype.deleteCurrentMove = function() {
    if (!this.currentMove.isRoot()) {
...
Gomoku.prototype.goToBeforeFirstMove = function() {
    while (!this.currentMove.isRoot()) {
        this.currentMove.recall();
    }
}

Gomoku.prototype.goToPreviousFork = function() {
    while (!this.currentMove.isRoot()) {
        this.currentMove.recall();
...
Gomoku.prototype.goToPreviousMove = function() {
    if (!this.currentMove.isRoot()) {
        this.currentMove.recall();
    }
}

Gomoku.prototype.goToNextMove = function() {
    var descendants = this.currentMove.descendants;
    for (var i = 0; i < descendants.length; i++) {
...
Gomoku.prototype.goToNextFork = function() {
    while (true) {
...
Gomoku.prototype.goToLastMove = function() {
    while (true) {
...
Gomoku.prototype.revert = function() {
...
Gomoku.prototype.reflectUpDown = function() {
    this.upDownReflected = !this.upDownReflected;
    this.__updateUi();
}

Gomoku.prototype.reflectLeftRight = function() {
    this.leftRightReflected = !this.leftRightReflected;
    this.__updateUi();
}

Gomoku.prototype.rotateClockwise = function() {
...
Gomoku.prototype.rotateCounterclockwise = function() {
    if (this.upDownReflected ^ this.leftRightReflected) {
        this.rotateTimes += 1;
    } else {
        this.rotateTimes += 3;
...
Gomoku.prototype.searchText = function() {
...
Gomoku.prototype.searchManual = function() {
...
Gomoku.prototype.__reinitializeMoves = function() {
...
Gomoku.prototype.__updateUi = function() {
    var boardInnerHtml = '<table class="board" background="/s/img/board.png" border="0" cellspacing="0" cellpadding="0">';
    for (var j = NUM_LINES - 1; j >= -1; j--) {
        ...
        for (var i = -1; i < NUM_LINES; i++) {
            ...
            if (i == -1 && j == -1) {
                ...
                boardInnerHtml += '<td class="boardSpot" id="i_' + x + '_' + y + '" onclick="__gomoku__.playNewMoveAt(' + x + ', ' + y + ');" onselectstart="return false;">&nbsp;</td>';
            }
        }
        boardInnerHtml += '</tr>';
    }
...
Gomoku.prototype.__getSearchData = function(upDownReflected, leftRightReflected, rotateTimes) {
    var part1 = new Array();
    var part2 = new Array();
...
Move.prototype.play = function() {
    __gomoku__.currentBranch.push(this);
    if (!this.__isPass()) {
        __gomoku__.stones[this.y][this.x] = this.stone;
    }
    if (this.__isPass() && !this.isRoot()) {
        __gomoku__.passHtmlElement.style.display = "block";
        setTimeout('__gomoku__.passHtmlElement.style.display="none"', 1000);
    }
    __gomoku__.currentMove = this;
    var parent = this.parent;
    parent.label = __gomoku__.labelHtmlElement.value;
    parent.comment = __gomoku__.commentHtmlElement.value;
    this.__play();
}
...
Move.prototype.recall = function() {
    ...
    this.label = __gomoku__.labelHtmlElement.value;
    this.comment = __gomoku__.commentHtmlElement.value;
    __gomoku__.currentBranch.pop();
    __gomoku__.currentMove = this.parent;
    if (!this.__isPass()) {
        __gomoku__.stones[this.y][this.x] = STONE.NONE;
    }
...
Move.prototype.__showAsMajor = function() {
    if (!this.__isPass()) {
        ...
        htmlElement.style.backgroundImage = (this.stone == STONE.BLACK ? 'url(/s/img/black_stone.png)' : 'url(/s/img/white_stone.png)');
...
Move.prototype.__showAsLast = function() {
    ...
    __gomoku__.labelHtmlElement.value = this.label;
    __gomoku__.commentHtmlElement.value = this.comment;
}
...
Move.prototype.__showAsVariant = function() {
    ...
    if (this.label == '') {
        if (this.stone == STONE.BLACK) {
            ...
            htmlElement.style.backgroundImage = 'url(/s/img/black_variant.png)';
        } else {
            htmlElement.innerHTML = '&nbsp;';
            htmlElement.style.color = '#000000';
            htmlElement.style.backgroundImage = 'url(/s/img/white_variant.png)';
        }
    } else {
        htmlElement.innerHTML = this.label;
        htmlElement.style.color = '#000000';
        htmlElement.style.backgroundImage = 'url(/s/img/block.png)';
    }
...
Move.prototype.__hide = function() {
    ...
    var htmlElement = this.__getHtmlElement();
    htmlElement.innerHTML = '&nbsp;';
    htmlElement.style.backgroundImage = 'url(/s/img/none.png)';
    htmlElement.onclick = function() {
        __gomoku__.playNewMoveAt(_x, _y);
    };
}
